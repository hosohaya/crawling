...
class Jra:
    ...
    def parse(self, view):
        ...
        venueId = self.venueModel.getVenueId()
        ...
        for i, record in enumerate(view.records):
            ...
            if horse == None:
                ...
                if self.only_null:
                    self.horseModel.updateOnlyNull()
                else:
            ...
            raceCodeId = self.recordModel.getRecordId()
            if raceCodeId == None:
                self.recordModel.save()
            else:
                self.recordModel.values['id'] = raceCodeId
                if view.getViewName() == 'result' or view.getViewName() == 'list':
                    self.recordModel.values['scrapy_flag'] = '1'
                # if self.only_null:
                #     self.recordModel.updateOnlyNull()
                # else:
        ...
        if all_number == True:
            self.recordModel.deleteNotRace(raceId)
        ...
        for i, payout in enumerate(view.payouts):
            ...
            if payoutId == None:
                ...
                # if self.only_null:
                #     self.payoutModel.updateOnlyNull()
                # else:
